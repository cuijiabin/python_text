...
if __name__ == '__main__':
    ...
    start = time.time()
    path_2 = 'C:/Users/liuwz/Desktop/课题/辰芯智能知识图谱/10数据/1/'
    df = pd.DataFrame()
    for file in os.listdir(path_2):
        f = open(path_2 + file, 'r', encoding='utf-8')
        content2 = f.read()
        f.close()
        events, spos = handler.phrase_ip(content2)
        spos = [i for i in spos if i[0] and i[2]]
        # for spo in spos:
        # print(spo)
        df1 = pd.DataFrame(spos)
        file1 = file.replace('.txt', '')
        df1.loc[:, 'lable'] = file1
        df = df.append(df1)
    print(df)
    wb = Workbook()
